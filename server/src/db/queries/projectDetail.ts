...
export function projectHeader(db: DB, projectPath: string): ProjectHeader | null {
  const head = db
    .prepare(
      `SELECT
         project_path, MAX(project_name) AS project_name,
  ...
  const totals30 = totalsSince(db, projectPath, 30);
  const totals7 = totalsSince(db, projectPath, 7);
  ...
  const primaryModel = db
    ...
    .get(projectPath) as { model: string } | undefined;

  const branches = db
    .prepare(
      `SELECT DISTINCT git_branch FROM sessions
       WHERE project_path = ? AND git_branch IS NOT NULL AND git_branch != ''`,
    )
    .all(projectPath) as Array<{ git_branch: string }>;
...
function totalsSince(db: DB, projectPath: string, days: number): number {
  const cutoff = new Date(Date.now() - days * 86_400_000).toISOString();
  const r = db
    .prepare(
      `SELECT COALESCE(SUM(t.input_tokens + t.output_tokens + t.cache_read_tokens + t.cache_creation_tokens), 0) AS total
       FROM sessions s JOIN turns t ON t.session_id = s.session_id
       WHERE s.project_path = ? AND t.ts >= ?`,
    )
    .get(projectPath, cutoff) as { total: number };
  return r.total;
}

export function projectCacheStats(db: DB, projectPath: string, days: number): ProjectCacheStats {
  const cutoff = new Date(Date.now() - days * 86_400_000).toISOString();
  const r = db
    .prepare(
      `SELECT
    ...
    .get(projectPath, cutoff) as { read: number; creation: number; input: number };
...
export function projectModelMix(
  ...
  const rows = db
    ...
    .all(projectPath, cutoff) as Array<{ model: string; tokens: number }>;
...
export function projectCacheTtl(db: DB, projectPath: string, days: number): CacheTtlSplit {
  const cutoff = new Date(Date.now() - days * 86_400_000).toISOString();
  const r = db
    .prepare(
      `SELECT
    ...
    .get(projectPath, cutoff) as { c5m: number; c1h: number };
...
export function projectEntrypoints(db: DB, projectPath: string): EntrypointStat[] {
  const rows = db
    .prepare(
      `SELECT COALESCE(entrypoint, 'unknown') AS entrypoint, COUNT(*) AS n
       FROM sessions WHERE project_path = ? GROUP BY entrypoint ORDER BY n DESC`,
    )
    .all(projectPath) as Array<{ entrypoint: string; n: number }>;
...
export function projectModelMixOverTime(
  ...
  const rows = db
    ...
    .all(projectPath, cutoff) as Array<{ date: string; model: string; tokens: number }>;
